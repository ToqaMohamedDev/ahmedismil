...
const mockCourses: Course[] = [
  {
    id: "1",
    title: "كورس تاريخ مصر الشامل",
    description: "كورس شامل يغطي جميع فترات تاريخ مصر من العصور القديمة حتى العصر الحديث",
    image: "/assets/ahmedismail.png",
    level: "first-prep",
    category: "كورسات مكثفة",
    lessonsCount: 25,
  },
  {
    id: "2",
    title: "مراجعة نهائية جغرافيا",
    description: "مراجعة شاملة لجميع موضوعات الجغرافيا للمرحلة الإعدادية",
    image: "/assets/ahmedismail.png",
    level: "third-prep",
...
export default function CoursesPage() {
  ...
  const availableCategories = useMemo(() => {
    ...
    // الحصول على التصنيفات التي لها كورسات في المرحلة المختارة
    const categoriesInLevel = new Set<string>();
    mockCourses.forEach((course) => {
      if (course.level === selectedLevel) {
        categoriesInLevel.add(course.category);
      }
    });
  ...
  // إذا تم اختيار المرحلة والتصنيف - عرض الكورسات
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <motion.div
        initial={{ opacity: 0, y: -20 }}
      ...
      {paginatedCourses.length > 0 ? (
        ...
        <EmptyState
          ...
          message="لم يتم العثور على كورسات في هذا التصنيف"
        />
      )}
    </div>
  );
